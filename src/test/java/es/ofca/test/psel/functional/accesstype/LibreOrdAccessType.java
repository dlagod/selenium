 ...
 * referencia al CASO de Test: http://xamp.sacyl.es/jira/browse/PSEL-18
 * 
 * @author dlago
 */
public class LibreOrdAccessType extends GeneralForm {

	
	private static final Logger LOGGER = Logger.getLogger(LibreOrdAccessType.class.getName());

	/**
	 * Método que crea una solicitud para una convocatoria Libre Ordinario.
	 * 
	 * @param userData
	 ...
	 */
	@Test(dataProvider = "datosConvocatoriaOK", groups = { "grupo.libre.ord.ok" })
	public void createLibreOrdAccessType(UserData userData) throws Exception {
		if (LOGGER.isInfoEnabled()) {
			LOGGER.info("Pruebas funcionales de referencia al CASO de Test: http://xamp.sacyl.es/jira/browse/PSEL-18");
			LOGGER.info("WebDriverSelenium.createLibreOrdAccessType");
		}
		...
		invokeMethods(userData, Constants.Methods.CLEAR, AccessType.ORD_FREE, false);

		// Se guarda
		save();

		// Se validan que salgan todos los campos obligatorios
		requiredValuesGeneralAccessType(userData, AccessType.ORD_FREE);

		// PASO 6
		//Se chequean los campos Requeridos
		invokeMethods(userData, Methods.FILL_REQUIRED, AccessType.ORD_FREE, false);

		//Se chequean los carácteres no válidos de los campos
		invokeMethods(userData, Methods.CHECK_VALUE, AccessType.ORD_FREE, false);
