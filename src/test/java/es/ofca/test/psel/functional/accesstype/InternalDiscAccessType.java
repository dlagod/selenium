 ...
 * referencia al CASO de Test: http://xamp.sacyl.es/jira/browse/PSEL-21
 * 
 * @author jluisf
 */
public class InternalDiscAccessType extends GeneralForm {
	
	private static final Logger LOGGER = Logger.getLogger(InternalDiscAccessType.class.getName());

	/**
	 * Método que crea una solicitud para una convocatoria Interna Discapacidad.
	 * 
	 * @param userData
	 *            DataSource que contiene los diferentes tipos de convocatoria
	 *            Interna Discapacidad
	 * @throws Exception
	 *             Excepción producida en el método
	 */
	@Test(dataProvider = "datosConvocatoriaOK", groups = { "grupo.internal.disc.ok" })
	public void createInternalDiscAccessType(UserData userData) throws Exception {
		if (LOGGER.isInfoEnabled()) {
			LOGGER.info("Pruebas funcionales de referencia al CASO de Test: http://xamp.sacyl.es/jira/browse/PSEL-21");
			LOGGER.info("WebDriverSelenium.createInternalDiscAccessType");
		}
		...
		invokeMethods(userData, Constants.Methods.CLEAR, AccessType.DISABILITY_INTERNAL, false);

		// Se guarda
		save();

		// Se validan que salgan todos los campos obligatorios
		requiredValuesGeneralAccessType(userData, AccessType.DISABILITY_INTERNAL);

		// PASO 6
		//Se chequean los campos Requeridos
		invokeMethods(userData, Methods.FILL_REQUIRED, AccessType.DISABILITY_INTERNAL, false);

		//Se chequean los carácteres no válidos de los campos
		invokeMethods(userData, Methods.CHECK_VALUE, AccessType.DISABILITY_INTERNAL, false);
			
		//PASO 7
		//Se rellenando todos los campos con valores válidos
		invokeMethods(userData, Methods.CLEAR, AccessType.DISABILITY_INTERNAL, false);
		invokeMethods(userData, Methods.FILL, AccessType.DISABILITY_INTERNAL, false);
	...
	public Object[][] createDataLoginOK() {
		...
		String[] roles = new String[] { Access.PROM_INTER_DISC.getValue() };
